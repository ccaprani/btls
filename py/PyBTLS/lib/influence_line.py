from .BTLS_collections import _InfluenceLine, _InfluenceSurface
...
class InfluenceLine():
    ...
    def __init__(self, IL_type:Literal["discrete","built-in","surface"]):
        ...
        self._data_dict = {"position":None, "ordinate":None, "type":None, "length":None, "inf_surf":None}
    ...
    def _set_IL_discrete(self, **kwargs) -> None:
        ...
        if compress_error is not None:
            if compress_error > 0.1:
                raise Warning("The compress_error is too large, which may cause the corresponding load effect significantly inaccurate.")
            position, ordinate = _compress_discrete_IL(position, ordinate, compress_error)

        if position is None or ordinate is None:
            raise ValueError("Uncompleted input for discrete influence line.")
        
        if not isinstance(position, (list, np.ndarray)) or not isinstance(ordinate, (list, np.ndarray)):
...
class InfluenceSurface():
    ...
    def set_IS(self, IS_matrix:Union[list,np.ndarray], lane_position:Union[list,np.ndarray]) -> None:
        """
        ...
        if not isinstance(lane_position, (list, np.ndarray)) or not isinstance(IS_matrix, (list, np.ndarray)):
            raise TypeError("The lane_position and IS_matrix should be list or np.ndarray.")
