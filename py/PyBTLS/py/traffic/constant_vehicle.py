...
from PyBTLS.py.utils_interface._helper_functions import data_enforce_type, read_default_file
...
class ConstantVehicle(_DfBased):
    ...
    def load_default(self):
        data = np.array(read_default_file('constant_vehicle.csv')[0:-1]) #Ignore last row as its empty
        columns = {
            "axle_spacing": float,
            "axle_weight": float,
        }
    ...
    def write_as_csv(self, path = "constant_vehicle.csv"):
