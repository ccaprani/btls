...
class CumulativeStatistics(_DfBased):
    ...
    def _from_txt_to_dataframe_kwargs(self, txt, *args, **kwargs):
        ...
        axle_header = [
            "event_" + str(i + 1) + "_truck" for i in range(0, num_defined_axles)
        ]
        axle_column_type = dict(
            zip(axle_header, np.array([float for i in range(0, num_defined_axles)]))
        )
...
class BlockMaximaSummary(_DfBased):
    ...
    def _from_txt_to_dataframe_kwargs(self, txt, *args, **kwargs):
        if isinstance(txt, str):
            ...
            txt = txt.split("\n")
            txt = [
                re.split("\t\t|\t", t)[0:-1] for t in txt
            ]  # Ignore empty column at the end. Ignore empty row at the end
            # Check if last row is empty
            if (len(txt[-1]) == 0) or ((len(txt[-1]) == 1) and (txt[-1][0] == "")):
                txt = txt[0:-1]
        data = txt
        ...
        # Append the axle info
        axle_header = [
            "event_" + str(i + 2) + "_truck" for i in range(0, num_defined_axles)
        ]
