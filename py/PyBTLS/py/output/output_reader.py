from PyBTLS.py.output.output import IntervalStatistics, CumulativeStatistics, BlockMaximaSummary
...
def read_interval_statistics_files(froot, bridge_length):
    ...
    fpath = fpath[filter]
    if len(fpath) == 0:
        raise ValueError("ERROR: No interval statistics file(s) was found in the `froot` directory for the `bridge_length` given")
    out = []
...
def read_cumulative_statistics_file(froot, bridge_length):
    ...
    fpath = fpath[filter]
    if len(fpath) == 0:
        raise ValueError("ERROR: No cumulative statistics file was found in the `froot` directory for the `bridge_length` given")
...
def read_block_maxima_summary_file(froot, bridge_length):
    ...
    fpath = fpath[filter]
    if len(fpath) == 0:
        raise ValueError("ERROR: No block maxima summary file(s) was found in the `froot` directory for the `bridge_length` given")
    out = []
    for i, f in enumerate(fpath):
        #Get LE number
        LE_num = str(int(f[len(froot)+5:].split("_")[2].split(".")[0]))
...
def read_block_maxima_event_separated_vehicles_file(froot, bridge_length, file_format):
    ...
    fpath = fpath[filter]
    if len(fpath) == 0:
        raise ValueError("ERROR: No block maxima separated by event file(s) was found in the `froot` directory for the `bridge_length` given")
    out = []
    for i, f in enumerate(fpath):
        out.append(BlockMaximaEvent(
            path = f, file_format = file_format)
        )
    return out
...
def read_block_maxima_event_mixed_vehicles_file(froot, bridge_length, file_format):
    ...
    fpath = fpath[filter]
    if len(fpath) == 0:
        raise ValueError("ERROR: No mixed block maxima file was found in the `froot` directory for the `bridge_length` given")
