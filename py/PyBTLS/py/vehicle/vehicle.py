...
from PyBTLS.py.utils_interface._vehicle_event_parser import parse_from_vehicle_text_list_to_df_kwargs
...
class Vehicle(_DfBased):
    def _from_txt_to_dataframe_kwargs(self, txt, *args, **kwargs):
        if not 'file_format' in kwargs:
            raise ValueError("ERROR: 'path' argument supplied but not 'file_format'. Please supply the 'file_format' of the vehicle text")
    ...
    def _check_args(self, *args, **kwargs):
        ...
        if ('path' in kwargs):
            if not (('path' in kwargs) and ('file_format' in kwargs)):
                raise ValueError("ERROR: Both 'path' and 'file_format' must be supplied together to import from file")
        if ('text' in kwargs):
            if not (('text' in kwargs) and ('file_format' in kwargs)):
                raise ValueError("ERROR: Both 'text' and 'file_format' must be supplied together to import from text")
        if ('file_format' in kwargs):
            if not (('text' in kwargs) and ('file_format' in kwargs) or ('path' in kwargs) and ('file_format' in kwargs)):
                raise ValueError("ERROR: 'file_format' supplied but not 'path' or 'text'. NOTE: Only supply 'text' OR 'path' along with 'file_format'")
        return 1
