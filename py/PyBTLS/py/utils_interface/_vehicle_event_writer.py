...
def vehicle_df_to_txt(df, file_format):
    ...
    if file_format == "MON":
        ...
        char_length = np.append(
            char_length, np.ones(num_defined_axles * 2, dtype=int) * 5
        )
        ...
        raise ValueError(
            "ERROR: file_format must be either 'CASTOR', 'BeDIT', 'DITIS', or 'MON'"
        )

    col_wise_data = [
        np.round(df.iloc[:, i].to_numpy() * factor[i]).astype(int).astype(str)
        for i in range(0, len(char_length))
    ]
    col_wise_data = [
        [col_wise_data[j][i].rjust(char_length[j]) for j in range(0, len(char_length))]
        for i in range(0, len(col_wise_data[0]))
    ]
