...
def parse_from_vehicle_text_list_to_df_kwargs(txt_list, file_format):
    ...
    if file_format in ["CASTOR", "BeDIT"]:
        ...
        column_width = np.append(
            column_width,
            np.array(
                [
                    [3, 2 if file_format == "CASTOR" else 3]
                    for i in range(0, num_defined_axles)
                ]
            ).flatten()[0:-1],
        )
        axle_header = np.array(
            [
                ["AW" + str(i + 1), "AS" + str(i + 1)]
                for i in range(0, num_defined_axles)
            ]
        ).flatten()[0:-1]
        header = np.append(header, axle_header)
        axle_column_type = dict(
            zip(
                axle_header,
                np.array(
                    [[float, float] for i in range(0, num_defined_axles)]
                ).flatten()[0:-1],
            )
        )
        ...
        num_defined_axles = int(
            excess_character / 10
        )  # Each axles are 5 + 5 character long

        # Append the axle info
        column_width = np.append(
            base_column_width, np.ones(num_defined_axles * 2, dtype=int) * 5
        )
        axle_header = np.array(
            [
                ["AW" + str(i + 1), "AS" + str(i + 1)]
        ...
        raise ValueError(
            "ERROR: file_format argument must be 'CASTOR', 'BeDIT', 'DITIS', or 'MON'"
        )
...
def parse_from_event_text_list_to_event_data(text):
    ...
    [
        [event_info_list[i].insert(0, day_list[i])]
        for i in range(0, len(event_info_list))
    ]
    # insert into the object directly. No output and assignment needed
    [
        [event_info_list[i].append(vehicle_list[i])]
        for i in range(0, len(event_info_list))
    ]
    # insert into the object directly. No output and assignment needed
