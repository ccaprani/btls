...
def assemble_garage(
    ...
    source_garage_format: int = 4,
    garage_size: int = 1000000,
    **kwargs,
) -> list[_Vehicle]:
    """
    ...
    if len(source_garage_path) != len(source_garage_percentage):
        raise ValueError(
            "The elements of source_garage_path and source_garage_percentage must be equal!"
        )
    ...
    for i in range(len(source_garage_path)):
        source_garage_dict[source_garage_path[i]] = load_garage_file(
            source_garage_path[i], source_garage_format, **kwargs
        )
        source_garage_no_vehicle_dict[source_garage_path[i]] = int(
            garage_size * source_garage_percentage[i]
        )

    for i in range(len(source_garage_path)):
        for _ in range(source_garage_no_vehicle_dict[source_garage_path[i]]):
            return_garage.append(
                random.choice(source_garage_dict[source_garage_path[i]])
            )
