...
__all__ = [
    "divide_by_regional_classification",
    "divide_by_no_axle",
    "divide_by_gvw_percentile",
    "divide_by_gvw_value",
    "divide_by_vehicle_length_percentile",
    "divide_by_vehicle_length_value",
    "write_divide_garage_file",
]
...
def divide_by_regional_classification(
    vehicle_list: list[_Vehicle], region_criterion: CriterionBase
) -> SubWIMDict:
    """
...
def divide_by_gvw_percentile(
    vehicle_list: list[_Vehicle], gvw_div_percentile_list: list
) -> (
    SubWIMDict
):  # np.percentile() is based on the raw data pdf, not the max and min values (Should be based on the max and min values)!
    """
    ...
    sub_WIM_dict = divide_by_gvw_value(
        vehicle_list, gvw_div_value_list, gvw_list=gvw_list
    )

    return sub_WIM_dict


def divide_by_gvw_value(
    vehicle_list: list[_Vehicle], gvw_div_value_list: list, **kwargs
) -> SubWIMDict:
    """
...
def divide_by_vehicle_length_percentile(
    vehicle_list: list[_Vehicle], vehicle_length_div_percentile_list: list
) -> (
    SubWIMDict
):  # np.percentile() is based on the raw data pdf, not the max and min values (Should be based on the max and min values)!
    """
    ...
    vehicle_length_div_value_list = _percentile_to_value(
        vehicle_length_div_percentile_list, vehicle_length_list
    )

    sub_WIM_dict = divide_by_vehicle_length_value(
        vehicle_list,
        vehicle_length_div_value_list,
        vehicle_length_list=vehicle_length_list,
    )

    return sub_WIM_dict


def divide_by_vehicle_length_value(
    vehicle_list: list[_Vehicle], vehicle_length_div_value_list: list, **kwargs
) -> SubWIMDict:
    """
    ...
    sub_WIM_dict = _divide_by_value(
        vehicle_list, vehicle_length_list, vehicle_length_div_value_list
    )
...
def _percentile_to_value(
    percentile_list: list[float], data: list[float]
) -> list[float]:
    """
...
def _divide_by_value(
    ...
    for div_value in div_value_list:
        gvw_index = [
            i for i, gvw in enumerate(vehicle_property_list_copy) if gvw <= div_value
        ]
...
def write_divide_garage_file(
    ...
    **kwargs,
    ...
    for key in sub_WIM_dict.data.keys():
        write_garage_file(
            sub_WIM_dict.data[key],
            output_folder_path + "/" + sub_WIM_dict.tag + "_" + key,
            out_garage_format,
            **kwargs,
        )
