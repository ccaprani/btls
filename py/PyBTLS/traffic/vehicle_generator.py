...
from ..pre_process.GarageProcessing import load_garage_file
...
class VehicleGenGarage:
    def __init__(self, garage: Union[Path, list[_Vehicle]], kernel: list[list[float]], garage_format: Literal[1,2,3,4] = None, **kwargs):
        """
        ...
        if isinstance(garage, (Path,str)):
            ...
            self._garage = Path(garage).resolve() if not isinstance(garage, Path) else garage.resolve()
    ...
    def _get_generator(
        ...
        vehicle_list = load_garage_file(self._garage, self._garage_format) if isinstance(self._garage, Path) else self._garage
