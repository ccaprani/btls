...
def main_func():
    ...
    bridge_1.add_load_effect(inf_line_surf=inf_surf_1, threshold=0.0)  # multi load effects test
    ...
    headway_gen_3 = pb.HeadwayGenCongested(congested_spacing=26.1, congested_speed=36.0, congested_gap_coef_var=0.05)
    ...
    traffic_gen.add_lane(vehicle_gen=vehicle_gen_1, headway_gen=headway_gen_1, lfc=lfc_1)
    traffic_gen.add_lane(vehicle_gen=vehicle_gen_1, headway_gen=headway_gen_2, lfc=lfc_2)
    traffic_gen.add_lane(vehicle_gen=vehicle_gen_2, headway_gen=headway_gen_3, lfc=lfc_3)
    traffic_gen.add_lane(vehicle_gen=vehicle_gen_3, headway_gen=headway_gen_4, lfc=lfc_4)
    traffic_gen.set_start_time(0.0)

    # or, use traffic loader
    traffic_loader = pb.TrafficLoader(no_lane=4)
    traffic_loader.add_traffic("./test_data/test_traffic_file.txt", file_format="MON", use_average_speed=False, use_const_speed=False, const_speed_value=36.0)
    ...
    output_config_1.set_BM_output(write_vehicle=True,write_summary=True,write_mixed=True)
    output_config_1.set_POT_output(write_vehicle=True,write_summary=True,write_counter=True)
    output_config_1.set_fatigue_output(write_fatigue_event=True,write_rainflow_output=True)
    output_config_1.set_stats_output(write_flow_stats=True,write_overall=True,write_intervals=True)
    
    output_config_2 = pb.OutputConfig()
    output_config_2.set_vehicle_file_output(write_vehicle_file=True,vehicle_file_format=4,vehicle_file_name="proof_traffic_file.txt")
    output_config_2.set_stats_output(write_flow_stats=True,write_overall=True,write_intervals=True)
...
def test_run():
    output_folder_list = ['Sim_1','Sim_2','Sim_3','Sim_4','Sim_5','Sim_6','Sim_7','Sim_8']
