...
def test_built_in_IL(built_in_id:int, built_in_length:float, cba_beam_string:str, cba_beam_position:float, cba_beam_LE:str):
    ...
    IL_cba.set_IL(position=position, ordinate=-ordinate) if built_in_id in [2, 8, 9] else IL_cba.set_IL(position=position, ordinate=ordinate)
    ...
    assert ((time_history_data["Effect 1"][1:-1] - time_history_data["Effect 2"][1:-1]).abs() <= 0.1).all()
...
def test_compress_IL():
    ...
    time_history_data = sim_output.read_data("time_history")["dir1"]
    assert ((time_history_data["Effect 1"] - time_history_data["Effect 2"]).abs() <= 0.1).all()
