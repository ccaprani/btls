...
from PyBTLS.py_module.output.output import _BtlsWrapper_DefaultOutputs, CumulativeStatistics, IntervalStatistics, BlockMaximaSummary
from PyBTLS.py_module.vehicle.vehicle import Vehicle
from PyBTLS.py_module.vehicle.events import BlockMaximaEvent
...
class BTLSWrapper():
    ...
    def read_outputs(self):
        if (self.options.analyse_statistics == 1) and (self.options.write_interval_statistics_files == 1):
            self.outputs.interval_statistics = self._read_interval_statistics_file()
        if (self.options.analyse_statistics == 1) and (self.options.write_cumulative_statistics_file == 1):
            self.outputs.cumulative_statistics = self._read_cumulative_statistics_file()
        ...
        if (self.options.analyse_block_max == 1) and (self.options.write_block_max_separated_vehicle_files == 1):
            self.outputs.block_maxima_separated_vehicles = self._read_block_maxima_event_separated_vehicles_file()
        if (self.options.analyse_block_max == 1) and (self.options.write_block_max_mixed_analysis == 1):
            self.outputs.block_maxima_mixed_vehicles = self._read_block_maxima_event_mixed_vehicles_file()
    ...
    def _read_interval_statistics_file(self):
        ...
        for i in range(0, N_LE):
            ...
            if len(fpath)>1:
                ...
                fpath = fpath[0]
            out.append(IntervalStatistics(
                descriptor = desc_name,
                path = fpath,
            ))
        return out

    def _read_cumulative_statistics_file(self):
        fpath = glob(self.working_dir+"/SS_C_*.txt") #There should only be one file with this name
        if len(fpath)>1:
            warnings.warn("WARNING: Multiple Interval Statistics file detected for some load effect. Ensure that the working directory was clear and there are no other bridge output files in the folder")
        else:
            fpath = fpath[0]
        return CumulativeStatistics(path = fpath)
    ...
    def _read_block_maxima_summary_file(self):
        ...
        for i in range(0, N_LE):
            ...
            if len(fpath)>1:
                ...
                fpath = fpath[0]
            out.append(BlockMaximaSummary(
                descriptor = desc_name, 
                path = fpath))
        return out

    def _read_block_maxima_event_separated_vehicles_file(self):
        out = []
        fpath_arr = np.array(glob(self.working_dir+"/BM_V_*_*.txt"))
        #Remove the one with All in the filename
        filter = [f.replace(self.working_dir, "") for f in fpath_arr]
        filter = [False if "All" in s else True for s in filter]
        fpath_arr = fpath_arr[filter]

        for i, fpath in enumerate(fpath_arr):
            out.append(BlockMaximaEvent(path = fpath, file_format = self.options.write_vehicle_file_format))
        return out

    def _read_block_maxima_event_mixed_vehicles_file(self):
        fpath = glob(self.working_dir+"/BM_V_*_All.txt")
        if len(fpath)>1:
            warnings.warn("WARNING: Multiple Block Maxima Summary file detected for some load effect. Ensure that the working directory was clear and there are no other bridge output files in the folder")
        else:
            fpath = fpath[0]
        return BlockMaximaEvent(path = fpath, file_format = self.options.write_vehicle_file_format)
    ...
    def _end_parallel_run(self):
        self.read_outputs()
        #Do not flush files here. Files will be purged by the parallel wrapper
    
    def run(self, purge_working_dir = False):
        self._check_input_completeness()
        self._create_working_dir()
        self.create_BTLSin_file()
        self.create_BTLS_bridge_files()
