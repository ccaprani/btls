...
void COutputManagerBase::OpenSummaryFiles()
{
	...
	{
		std::string file;
		file = btls::outPath(m_OutputDir, m_FileStem + "_S_" + to_string(m_BridgeLength) + "_Eff_" + to_string(i+1) + ".txt");
...
{
	std::string thefile;
	thefile = btls::outPath(m_OutputDir, m_FileStem + "_V_" + to_string(m_BridgeLength) + "_" + to_string(i) + ".txt");
