...
public:
    ...
    size_t Road_NO_LANES_DIR1;  // assigned in PrepareSim
    size_t Road_NO_LANES_DIR2;  // assigned in PrepareSim
    size_t Road_NO_LANES;  // assigned in PrepareSim
    size_t Road_NO_DIRS;  // assigned in PrepareSim
    ...
    double Gen_NO_OVERLAP_LENGTH;  // assigned in PrepareSim
    ...
    int Output_WRITE_EVENT_BUFFER_SIZE = 1000;
    ...
    int Output_VehicleFile_WRITE_VEHICLE_BUFFER_SIZE = 1000;
    ...
    int Output_BlockMax_WRITE_BM_BUFFER_SIZE = 1000;
    ...
    int Output_POT_WRITE_POT_BUFFER_SIZE = 1000;
    ...
    int Output_Stats_WRITE_SS_BUFFER_SIZE = 1000;
    ...
    void FlowGenNHM(int classification);
    void FlowGenConstant(int classification);
    ...
    void OutputGeneral(bool write_time_history=false, bool write_each_event=false, bool write_fatigue_event=false, int write_buffer_size=1000);
    void OutputVehicleFile(bool write_vehicle_file=false, unsigned int vehicle_file_format=4, std::string vehicle_file_name="Vehicles.txt", int buffer_size=1000, bool write_flow_stats=false);
    void OutputBlockMax(bool write_blockmax=false, bool write_vehicle=false, bool write_summary=false, bool write_mixed=false, int block_size_days=1, int block_size_secs=0, int buffer_size=1000);
    void OutputPOT(bool write_pot=false, bool write_vehicle=false, bool write_summary=false, bool write_counter=false, int pot_size_days=1, int pot_size_secs=0, int buffer_size=1000);
    void OutputStats(bool write_stats=false, bool write_cumulative=false, bool write_intervals=false, int interval_size=3600, int buffer_size=1000);
